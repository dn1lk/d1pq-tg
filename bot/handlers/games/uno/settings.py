...
def get_current_difficulty(message: types.Message):
    difficulty = message.entities[2]
    return message.text[difficulty.offset:difficulty.offset + difficulty.length]


@router.callback_query(k.Games.filter(F.value == 'difficulty'), F.from_user.id == F.message.entities[1].user.id)
...
@router.callback_query(difficulty_change_filter, F.from_user.id == F.message.entities[1].user.id)
...
@router.callback_query(k.Games.filter(F.value == 'back'), F.from_user.id == F.message.entities[1].user.id)
...
async def difficulty_no_owner_handler(query: types.CallbackQuery):
    await query.answer(_("You cannot setting this game."))
