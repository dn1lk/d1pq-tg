...
from worker.handlers import get_command_list, USERNAME, NO_ARGS
...
async def hello_handler(message: types.Message, bot: Bot, i18n: I18n):
    await message.answer(
        _(
            ...
            username=USERNAME.format(id=message.from_user.id, name=message.from_user.username)
...
async def settings_handler(message: types.Message, bot: Bot):
    ...
    from .settings.start import get_settings_answer

    await message.answer(**await get_settings_answer(message.chat.id, message.from_user.id, bot))
